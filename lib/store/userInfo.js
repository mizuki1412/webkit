...
export function initStoreUserInfo() {
  const dataJson = localStorage.getItem(import.meta.env.VITE_APP_NAME + '-user');
  if (!dataJson) {
...
export function updateStoreUserInfo(data) {
  ...
  localStorage.setItem(import.meta.env.VITE_APP_NAME + '-user', JSON.stringify(obj));
}
...
export function rmStoreUserInfo() {
  ...
  localStorage.removeItem(import.meta.env.VITE_APP_NAME + '-user');
}
